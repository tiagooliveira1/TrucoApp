package com.example.troli.trucoapp;
        ...
        mAuth.createUserWithEmailAndPassword(login, senha).addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if(task.isSuccessful()) {
                    ...
                    mEmailView.setText(task.getResult().getUser().getEmail().toString());
                } else {
                    Log.e("", "onComplete: Failed=" + task.getException().getMessage());
                    Toast.makeText(LoginActivity.this, "Erro ao criar o mizerável."+task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                }
            }
        });
